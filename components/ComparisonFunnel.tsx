...
import { 
  ...
  Check, CreditCard, MapPin, Hash, CreditCard as CardIcon, FileCheck,
  Info, ChevronDown, TrendingDown, FileText, Banknote
} from 'lucide-react';
...
// Elenco province principali per il dropdown
...
const MOCK_OFFERS: (OfferLuce | OfferGas | OfferDual)[] = [
  { id: '1', provider_id: 'p1', nome_offerta: 'Eco-Green Fix', tipo_tariffa: 'fissa', prezzo_kwh: 0.12, quota_fissa_mensile: 10, bonus_attivazione: 50, green_energy: true, penale_recesso: false, visibile: true, created_at: '', provider: { id: 'p1', nome: 'GreenNetwork', attivo: true, created_at: '' } } as OfferLuce,
  { id: '2', provider_id: 'p2', nome_offerta: 'Gas Safe Plus', tipo_tariffa: 'fissa', prezzo_smc: 0.45, quota_fissa_mensile: 12, bonus_attivazione: 30, penale_recesso: false, visibile: true, created_at: '', provider: { id: 'p2', nome: 'Plenitude', attivo: true, created_at: '' } } as OfferGas,
  { id: '3', provider_id: 'p3', nome_offerta: 'Combo Relax', prezzo_kwh: 0.11, prezzo_smc: 0.42, quota_fissa_mensile: 18, bonus_attivazione: 80, visibile: true, created_at: '', provider: { id: 'p3', nome: 'Eni', attivo: true, created_at: '' } } as OfferDual
];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col bg-white rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] overflow-hidden border border-slate-100">
        ...
        {/* Nuova spesa annua + Risparmio evidenziato */}
        ...
        <div className="p-8 flex flex-col items-center justify-center bg-white">
          ...
          <div className="flex items-center gap-2 text-emerald-600">
            <span className="text-3xl font-black mb-2">€</span>
            <span className="text-6xl font-black tracking-tight">{result.nuova_spesa_mensile}</span>
          </div>
        </div>
      </div>

      {/* Sezione Sblocco - ORA BLU CON TESTO RAFFINATO */}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3 px-2">
        <div className="flex items-center gap-3 text-sm text-slate-600"><div className="w-5 h-5 rounded-full bg-emerald-100 flex items-center justify-center shrink-0"><Check size={12} className="text-emerald-600" /></div> Fornitura 100% Green inclusa</div>
        <div className="flex items-center gap-3 text-sm text-slate-600"><div className="w-5 h-5 rounded-full bg-emerald-100 flex items-center justify-center shrink-0"><Check size={12} className="text-emerald-600" /></div> Prezzo bloccato per 12 mesi</div>
      </div>
...
const Step7Anagrafica = ({ leadData, updateLeadData, bestOffer, onSubmit, isSaving }: any) => {
  const [form, setForm] = useState<ContractData & { nome: string; cognome: string }>({
    ...
    codice_fiscale: '', indirizzo: '', cap: '', citta: '', provincia: 'MI',
    pod: '', pdr: '', metodo_pagamento: 'iban', iban: ''
  });
  ...
  return (
    <div className="space-y-8">
      <div className="text-center">
        <h2 className="text-xl md:text-2xl font-black text-slate-900 leading-tight md:whitespace-nowrap">Ricevi l'offerta a te dedicata</h2>
...
const ComparisonFunnel: React.FC = () => {
  ...
  const [data, setData] = useState<LeadData>({ tipo_cliente: 'privato', tipo_servizio: 'luce', spesa_mensile: 0, consumo_luce_kwh: 0, consumo_gas_smc: 0, email: '', telefono: '', nome: '', cognome: '' });
  ...
  const handleCalculate = async (result: ComparisonResult) => {
    ...
    try {
      const { data: lead, error } = await supabase.from('leads').insert([{
        tipo_cliente: data.tipo_cliente, email: data.email, origine: 'manual', step_corrente: 'comparison', stato: 'parziale'
      }]).select().single();
  ...
  const handleUpdatePhoneAndProceed = async () => {
    ...
    try {
      ...
      setCurrentStep(6);
    } catch (err) { setCurrentStep(6); } finally { setIsSaving(false); }
  };
  ...
  const handleFinalSubmit = async (contract: ContractData) => {
    ...
    try {
      ...
      await supabase.from('contracts_pre').insert([{
        lead_id: leadId, offer_id: bestOffer.offer_id, tipo_pratica: 'cambio', indirizzo_fornitura: contract.indirizzo, cap: contract.cap, citta: contract.citta, provincia: contract.provincia, stato: 'in_attesa'
      }]);
      await supabase.from('leads').update({ stato: 'inviato', step_corrente: 'anagrafica', nome: data.nome, cognome: data.cognome }).eq('id', leadId);
      await EmailService.sendContractEmails(data, contract, bestOffer.offer_name);
      setCurrentStep(100);
    } catch (err) { setCurrentStep(100); } finally { setIsSaving(false); }
  };
